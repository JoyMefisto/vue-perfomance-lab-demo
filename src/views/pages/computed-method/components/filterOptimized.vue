...
const filtered = computed(() => items.filter((x) => x.includes(query.value)))
...
<template>
  <section class="border border-cyan-200 p-4 rounded-lg">
    <h3 class="font-bold text-cyan-700 mb-2">✅ Filter — Optimized (computed)</h3>
    ...
    <div class="flex gap-2 items-center mb-2">
      ...
      <button
        @click="runTest"
        :disabled="loading"
        class="bg-cyan-500 text-white px-3 py-1 rounded disabled:opacity-50 cursor-pointer"
      >
        Run
      </button>
      <span v-if="loading" class="text-cyan-700 font-semibold">Process...</span>
    </div>
